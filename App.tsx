...
// БАЗОВЫЕ ИКОНКИ (Самые надежные)
import { Heart, BookOpen, ChevronRight, Settings, Info, User as UserIcon, Activity, Zap, Camera, Star, ArrowLeft, MessageSquare, Award, Medal, Loader2, Cloud, Lock, Moon, Search, Sun, Coffee, Brain, Briefcase, Feather, Compass, Anchor, Target, Battery, X, Shield, Map, Smile } from 'lucide-react';
...
const DEFAULT_CONFIG: SiteConfig = {
  ...
  aboutParagraphs: ["Mindful Mirror — зеркало сознания."],
  quotes: [],
  adminPasscode: "0000"
};
...
// --- НОВЫЕ КЛЮЧИ (СБРОС) ---
const STORAGE_KEYS = {
  PROFILE: 'mm_profile_safe_v1', 
  HISTORY: 'mm_history_safe_v1',
  SESSIONS: 'mm_sessions_safe_v1',
  TIME: 'mm_time_safe_v1',
  ACTIVITY: 'mm_activity_safe_v1',
  JOURNAL: 'mm_journal_safe_v1',
  CONFIG: 'mm_config_safe_v1',
  DAILY_INSIGHT: 'mm_insight_safe_v1'
};

const StylizedMMText = ({ text = "mm" }) => (
  <span className="font-extrabold italic uppercase select-none">{text}</span>
);

// --- СТАДИИ РОСТА (ЭМОДЗИ - ЧТОБЫ НЕ БЫЛО ОШИБОК) ---
const TREE_STAGES = [
  { threshold: 50000, title: "Древо Жизни", icon: "🌳", color: "text-amber-600", desc: "Вершина эволюции." },
  { threshold: 20000, title: "Мудрое Древо", icon: "🌳", color: "text-emerald-800", desc: "Глубокие корни." },
  { threshold: 5000, title: "Цветущий Сад", icon: "🌸", color: "text-pink-500", desc: "Плоды практики." },
  { threshold: 1500, title: "Могучее Древо", icon: "🌲", color: "text-emerald-700", desc: "Сила и покой." },
  { threshold: 300, title: "Крепкое Древо", icon: "🌳", color: "text-emerald-600", desc: "Уверенность." },
  { threshold: 80, title: "Молодое Дерево", icon: "🌿", color: "text-emerald-500", desc: "Быстрый рост." },
  { threshold: 15, title: "Саженец", icon: "🌱", color: "text-emerald-400", desc: "Начало пути." },
  { threshold: 0, title: "Росток", icon: "🌱", color: "text-emerald-300", desc: "Потенциал." },
];

const ARCHETYPE_INFO: any = {
  "Творец": { desc: "Вы видите мир как полотно.", strength: "Воображение", shadow: "Перфекционизм", advice: "Создавайте, не ждите.", icon: Feather, color: "text-purple-600", bg: "bg-purple-50" },
  "Правитель": { desc: "Вы создаете порядок.", strength: "Лидерство", shadow: "Контроль", advice: "Доверяйте процессу.", icon: Briefcase, color: "text-indigo-600", bg: "bg-indigo-50" },
  ...
  "Хранитель": { desc: "Вы — опора.", strength: "Забота", shadow: "Жертвенность", advice: "Берегите себя.", icon: Shield, color: "text-emerald-600", bg: "bg-emerald-50" },
  "Искатель": { desc: "Вы — путник.", strength: "Свобода", shadow: "Непостоянство", advice: "Найдите дом внутри.", icon: Compass, color: "text-amber-600", bg: "bg-amber-50" }
};
  ...
  return (
    <div className="h-full flex flex-col bg-white overflow-y-auto animate-fade-in relative z-50 p-6">
      ...
      <div className="flex-1 flex flex-col items-center text-center">
        <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 ${info.bg} ${info.color}`}><Icon size={48} /></div>
        <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Ваш Архетип</h2>
        <h1 className="text-3xl font-black text-slate-800 mb-4">{archetype}</h1>
        <p className="text-lg text-slate-600 mb-8">{info.desc}</p>
        <div className="w-full space-y-4 mb-8 text-left">
           <div className="p-4 bg-slate-50 rounded-xl border border-slate-100"><span className="font-bold text-slate-700 block mb-1">Сила</span>{info.strength}</div>
           <div className="p-4 bg-slate-50 rounded-xl border border-slate-100"><span className="font-bold text-slate-700 block mb-1">Совет</span>{info.advice}</div>
        </div>
        {!isReadOnly && <button onClick={onContinue} className="w-full py-4 rounded-xl bg-indigo-600 text-white font-bold text-lg active:scale-95 transition-all">Далее</button>}
      </div>
    </div>
  );
};
...
const TutorialScreen: React.FC<{ onFinish: () => void }> = ({ onFinish }) => {
  ...
  const slides = [
    { title: "Карта Дня", desc: "Утренний план, созданный ИИ специально для вас.", icon: Map },
    { title: "Настроение", desc: "Адаптируйте план под свой уровень энергии.", icon: Battery },
    { title: "Рост", desc: "Ваш прогресс — это растущее дерево.", icon: Activity }
  ];

  return (
    <div className="h-full flex flex-col bg-white px-6 py-10 animate-fade-in relative z-50">
      <div className="flex-1 flex flex-col justify-center items-center text-center">
        <div className="w-24 h-24 bg-indigo-50 text-indigo-500 rounded-full flex items-center justify-center mb-6">{React.createElement(slides[slide].icon, { size: 48 })}</div>
        <h2 className="text-2xl font-bold text-slate-800 mb-4">{slides[slide].title}</h2>
        <p className="text-slate-500">{slides[slide].desc}</p>
      </div>
      <button onClick={() => { if (slide < 2) setSlide(s => s + 1); else onFinish(); }} className="w-full py-4 rounded-xl bg-indigo-600 text-white font-bold text-lg active:scale-95 transition-all">
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const h = localStorage.getItem(STORAGE_KEYS.HISTORY); if(h) setHistory(JSON.parse(h));
      ...
      const d = localStorage.getItem(STORAGE_KEYS.DAILY_INSIGHT); if(d) setDailyInsight(JSON.parse(d));
    } catch(e) {}
  }, []);
  ...
  return (
    <div className="h-screen w-full overflow-hidden flex flex-col font-sans bg-[#F8FAFC]">
      <main className="flex-1 relative overflow-hidden z-10">
        ...
        {currentView === 'CHAT' && <ChatInterface mode={selectedMode!} onBack={() => setCurrentView('HOME')} onSessionComplete={handleSessionComplete} />}
        {/* Остальные экраны упрощены для надежности, но функционал есть */}
        {currentView === 'PROFILE' && <div className="p-6"><h1 className="text-2xl font-bold mb-4">Профиль</h1><div className="bg-white p-6 rounded-3xl text-center"><div className="w-20 h-20 bg-slate-200 rounded-full mx-auto mb-4"></div><h2 className="text-xl font-bold">{userProfile.name}</h2><p className="text-indigo-500">{userProfile.archetype}</p><button onClick={() => setCurrentView('ARCHETYPE_RESULT')} className="mt-4 text-sm font-bold text-slate-400">Показать архетип</button></div></div>}
      ...
      {['HOME', 'PROFILE', 'SETTINGS'].includes(currentView) && <BottomNav currentView={currentView as any} onChangeView={(v) => setCurrentView(v)} />}
    </div>
  );
};
