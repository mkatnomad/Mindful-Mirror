...
// БАЗОВЫЕ ИКОНКИ (100% безопасные)
import { Heart, BookOpen, ChevronRight, Settings, Info, User as UserIcon, Activity, Quote, Clock, Zap, Camera, Star, ArrowLeft, MessageSquare, Award, Medal, RefreshCw, Loader2, Cloud, Lock, Moon, Search, Sparkles, Sun, Coffee, Brain, Briefcase, Feather, Compass, Anchor, Target, Battery, X, Shield, Map, Smile, Lightbulb, CheckCircle, Leaf, Sprout } from 'lucide-react';
...
// --- КЛЮЧИ СБРОСА (ЭТО ПОЧИНИТ БЕЛЫЙ ЭКРАН) ---
const STORAGE_KEYS = {
  PROFILE: 'mm_profile_golden_v1', 
  HISTORY: 'mm_history_golden_v1',
  SESSIONS: 'mm_sessions_golden_v1',
  TIME: 'mm_time_golden_v1',
  ACTIVITY: 'mm_activity_golden_v1',
  JOURNAL: 'mm_journal_golden_v1',
  CONFIG: 'mm_config_golden_v1',
  DAILY_INSIGHT: 'mm_insight_golden_v1'
};
...
// --- ВЕКТОРНЫЕ ДЕРЕВЬЯ (Твои любимые SVG) ---
const TreeIllustration: React.FC<{ stage: number, className?: string }> = ({ stage, className }) => {
  ...
  return (
    <svg viewBox="0 0 100 100" className={className} fill="none">
      <defs>
        <radialGradient id={`grad-${stage}`} cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
      ...
      <circle cx="50" cy="50" r="48" fill={`url(#grad-${stage})`} />
