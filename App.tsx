...
// Импортируем только самые базовые иконки, которые есть в любой версии
import { Heart, BookOpen, ChevronRight, Settings, Info, User, Activity, Zap, Camera, Star, ArrowLeft, MessageSquare, Award, Medal, Loader2, Cloud, Moon, Sun, Coffee, Brain, Briefcase, Feather, Compass, Anchor, Target, Battery, X, Shield, Map, Smile } from 'lucide-react';
...
// Этот компонент поймает ошибку вместо белого экрана
class ErrorBoundary extends React.Component<{ children: React.ReactNode }, { hasError: boolean, error: string }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="p-8 flex flex-col items-center justify-center h-screen bg-red-50 text-red-900 text-center">
          <h1 className="text-2xl font-bold mb-4">Упс! Ошибка.</h1>
          <p className="mb-6 font-mono text-sm bg-white p-4 rounded border border-red-200">{this.state.error}</p>
          <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="px-6 py-3 bg-red-600 text-white rounded-xl font-bold shadow-lg">
            СБРОСИТЬ И ПОЧИНИТЬ
          </button>
        </div>
      );
...
// КЛЮЧ СБРОСА - ЕСЛИ ВЫ ПОМЕНЯЕТЕ ЦИФРУ, ПРИЛОЖЕНИЕ ОБНУЛИТСЯ
const RESET_KEY = 'mm_reset_v999'; 

const STORAGE_KEYS = {
  PROFILE: 'mm_profile_final', 
  HISTORY: 'mm_history_final',
  SESSIONS: 'mm_sessions_final',
  TIME: 'mm_time_final',
  ACTIVITY: 'mm_activity_final',
  JOURNAL: 'mm_journal_final',
  CONFIG: 'mm_site_config',
  DAILY_INSIGHT: 'mm_insight_final'
};

const StylizedMMText = ({ text = "mm" }) => <span className="font-extrabold italic uppercase select-none font-sans">{text}</span>;
const Logo = ({ className = "w-20 h-20" }: { className?: string }) => <img src="/logo.png" alt="MM" className={`${className} object-contain`} onError={(e) => e.currentTarget.style.display = 'none'} />;

// --- ВЕКТОРНЫЕ ДЕРЕВЬЯ (SVG) ---
// Чистый SVG без внешних иконок, чтобы не ломалось
...
const ArchetypeResultScreen: React.FC<{ archetype: string, onContinue: () => void, isReadOnly?: boolean, onBack?: () => void }> = ({ archetype, onContinue, isReadOnly, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-white overflow-y-auto animate-fade-in relative z-50">
      <div className="p-6 pb-0">{isReadOnly && <button onClick={onBack} className="p-2 -ml-2 text-slate-400 mb-4"><ArrowLeft size={24} /></button>}</div>
...
const TutorialScreen: React.FC<{ onFinish: () => void }> = ({ onFinish }) => {
  ...
  const slides = [
    ...
    { title: "Настроение", desc: "Нажмите на кнопку 'Как ты?' на главной, чтобы адаптировать план.", icon: Battery, color: "text-emerald-500", bg: "bg-emerald-50" },
    { title: "Древо Сознания", desc: "Ваш прогресс визуализируется в виде дерева.", icon: TreeDeciduous, color: "text-amber-500", bg: "bg-amber-50" }
  ];
  return (
...
// --- APP COMPONENT ---
const App: React.FC = () => {
  // АВТО-СБРОС ПРИ ПЕРВОМ ЗАПУСКЕ ЭТОЙ ВЕРСИИ
  useEffect(() => {
  ...
  const getTreeStage = (s: number) => TREE_STAGES.find(r => s >= r.threshold) || TREE_STAGES[0];
    ...
    <div className="h-full overflow-y-auto px-6 pt-4 pb-32">
       ...
       <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm flex items-center justify-between" onClick={() => setCurrentView('RANKS_INFO')}>
          <div className="flex items-center space-x-4">
            <div className="w-12 h-12"><TreeIllustration stage={currentTree.stageIndex} className="w-full h-full" /></div>
    ...
    <div className="h-screen w-full overflow-hidden flex flex-col font-sans bg-[#F8FAFC]">
      <ErrorBoundary>
        <main className="flex-1 relative overflow-hidden z-10">
          ...
          {currentView === 'SETTINGS' && <div className="p-6"><h1 className="text-2xl font-bold mb-4">Настройки</h1><button onClick={() => { if(window.confirm('Сброс?')) { localStorage.clear(); window.location.reload(); } }} className="w-full p-4 bg-red-50 text-red-500 rounded-xl font-bold">Сбросить всё</button></div>}
        </main>
