...
// --- ИСПРАВЛЕНИЕ: ДОБАВЛЕН Search В СПИСОК ИМПОРТА ---
import { Search, Heart, BookOpen, ChevronRight, Settings, Info, User as UserIcon, Activity, Quote, Clock, Zap, Camera, Star, ArrowLeft, MessageSquare, Award, Medal, RefreshCw, Loader2, Cloud, Lock, Moon, Sun, Coffee, Brain, Briefcase, Feather, Compass, Anchor, Target, Battery, X, Shield, Map, Smile, Lightbulb, CheckCircle, Leaf, Sprout, TreeDeciduous } from 'lucide-react';
...
// КЛЮЧИ ДЛЯ ПОЛНОГО СБРОСА (УДАЛЯТ ОШИБКУ)
const STORAGE_KEYS = {
...
// --- ВЕКТОРНЫЕ ДЕРЕВЬЯ (ВАШ ЛЮБИМЫЙ ДИЗАЙН) ---
...
const ARCHETYPE_INFO: any = {
  ...
  "Мудрец": { desc: "Вы ищете истину. Ваша суть — понимание законов мира.", strength: "Интеллект и аналитика", shadow: "Бездействие из-за вечного анализа", advice: "Знание без действий бесполезно.", icon: BookOpen, color: "text-blue-600", bg: "bg-blue-50" },
  "Хранитель": { desc: "Вы — опора. Ваша суть — забота, эмпатия и служение.", strength: "Сострадание и надежность", shadow: "Самопожертвование и обида", advice: "Сначала наденьте маску на себя.", icon: Shield, color: "text-emerald-600", bg: "bg-emerald-50" },
...
const OnboardingScreen: React.FC<{ onComplete: (data: Partial<UserProfile>) => void, onBack: () => void }> = ({ onComplete, onBack }) => {
  ...
  const steps = [
    ...
    { title: "Новизна?", type: 'archetype', options: [{ label: "Восторг", type: 'EXPLORER', icon: Search }, { label: "Любопытство", type: 'SAGE', icon: Search }, { label: "Польза", type: 'RULER', icon: Briefcase }, { label: "Осторожность", type: 'CAREGIVER', icon: Lock }] },
...
// --- MAIN APP ---
const App: React.FC = () => {
  // АВТО-СБРОС ПРИ ПЕРВОМ ЗАПУСКЕ
  useEffect(() => {
    const hasReset = localStorage.getItem('mm_emergency_reset_v1');
    if (!hasReset) {
      localStorage.clear();
      localStorage.setItem('mm_emergency_reset_v1', 'true');
    ...
    <div className="p-6 pt-12 h-full overflow-y-auto animate-fade-in relative z-10 pb-24">
      ...
      <div className="bg-white shadow-sm border-slate-100 rounded-[32px] p-8 border flex flex-col items-center text-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center w-full">
          ...
          <div className="w-full pt-8 mt-10 border-t border-slate-100 flex justify-around cursor-pointer"><div className="text-center"><p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider mb-1">Версия</p><p className="text-base font-semibold text-slate-700">6.0.0</p></div><div className="text-center"><p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider mb-1">Сборка</p><p className="text-base font-semibold text-slate-700">FIXED</p></div></div>
