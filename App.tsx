...
const STORAGE_KEYS = {
  ...
  DAILY_INSIGHT: 'mm_daily_insight_v10' // v10 - Обновление под настроение
};
...
// --- КОМПОНЕНТ ДЕРЕВА РОСТА ---
const GrowthTree: React.FC<{ points: number }> = ({ points }) => {
  // Логика стадий роста
  let stage = { icon: Sprout, label: "Семя", color: "text-emerald-300", size: 32 };
  
  if (points > 50) stage = { icon: Sprout, label: "Росток", color: "text-emerald-400", size: 40 };
  if (points > 200) stage = { icon: Leaf, label: "Молодое дерево", color: "text-emerald-500", size: 48 };
  if (points > 500) stage = { icon: TreeDeciduous, label: "Крепкое дерево", color: "text-emerald-600", size: 56 };
  if (points > 1500) stage = { icon: TreeDeciduous, label: "Мудрое дерево", color: "text-emerald-700", size: 64 };
    ...
    <div className="w-full bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 p-6 rounded-[32px] flex items-center justify-between shadow-sm relative overflow-hidden">
       ...
       <div className="relative z-10 flex items-center space-x-5">
          ...
          <div>
             ...
             <p className="text-xs text-slate-400 mt-1 font-medium">{points} очков осознанности</p>
          </div>
       </div>
    </div>
  );
  ...
  const handleSelect = (option: any) => {
    ...
    if (step < steps.length - 1) {
      ...
      onComplete({ 
        ...
        ...finalData,
        [currentStepData.key!]: option.value // Save last answer
      });
    }
  };

  // Hack for simpler array access
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const generateDailyAdvice = async () => {
      ...
      // Если совет уже есть на сегодня И он сгенерирован для этого же настроения - не трогаем
      if (dailyInsight && dailyInsight.date === todayStr && dailyInsight.generatedForMood === currentMood) {
        return;
      }
      ...
      try {
        ...
        let moodInstruction = "";
        if (currentMood === 'low') moodInstruction = "КЛИЕНТ УСТАЛ. Дай очень мягкие, заботливые советы. Никакого достигаторства. Фокус на отдыхе.";
        if (currentMood === 'high') moodInstruction = "КЛИЕНТ НА ПИКЕ. Дай амбициозные задачи. Можно челлендж.";
        if (currentMood === 'flow') moodInstruction = "КЛИЕНТ В ПОТОКЕ. Помоги удержать это состояние и направить на творчество.";
        
        const prompt = `
          Ты — мудрый ментор. Клиент: ${userName}.
          Архетип: "${userProfile.archetype}". Цель: "${userProfile.focus}".
          
          ВАЖНО - СОСТОЯНИЕ КЛИЕНТА: ${moodInstruction}
          
          Составь Карту Дня. Конкретно, без воды.
          Разделитель: "|||". НЕ пиши заголовки (Мышление:, Действие: и т.д.), пиши сразу текст.
          
          1. МЫШЛЕНИЕ (Короткая установка на день).
          2. ДЕЙСТВИЕ (Один шаг к цели).
          3. ТЕЛО (Как поддержать энергию с учетом состояния).
        ...
        // Чистим мусор если ИИ все же добавил заголовки
  ...
  const handleMoodSelect = (mood: 'high' | 'flow' | 'ok' | 'low') => {
    ...
    // 2. Обновляем профиль (это триггернет useEffect и перегенерирует совет)
          ...
          <div className="w-full relative overflow-hidden rounded-[32px] bg-white border border-slate-100 p-6 text-left shadow-lg shadow-indigo-100/50">
             ...
             <div className="relative z-10">
               <div className="flex justify-between items-start mb-4">
                 <span className="bg-indigo-50 text-indigo-600 text-[10px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider">На сегодня</span>
                 
                 {/* КНОПКА БАТАРЕЙКИ ВНУТРИ КАРТЫ */}
                 <button onClick={(e) => { e.stopPropagation(); setIsBatteryModalOpen(true); }} className="flex items-center space-x-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-full transition-colors active:scale-90">
                    <Battery size={14} className={userProfile.currentMood === 'low' ? "text-rose-500" : "text-emerald-500"} />
                    <span className="text-[10px] font-bold text-slate-600">
                      {userProfile.currentMood === 'high' ? 'На пике' : userProfile.currentMood === 'low' ? 'На нуле' : userProfile.currentMood === 'flow' ? 'В потоке' : 'Норм'}
                    </span>
                 </button>
               </div>
               ...
               <div className="mb-6 min-h-[60px]">
                 {isInsightLoading ? (
                   ...
                   <h2 className="text-xl font-bold text-slate-800 leading-snug line-clamp-3">{dailyInsight?.mindset || "Загрузка..."}</h2>
                 )}
               </div>
               ...
               <button onClick={() => setCurrentView('DAILY_GUIDE')} className="w-full py-3.5 rounded-2xl bg-indigo-600 text-white font-bold text-sm shadow-md shadow-indigo-200 active:scale-95 transition-all flex items-center justify-center space-x-2 group">
                 <Map size={16} /><span>Открыть карту дня</span><ChevronRight size={16} className="opacity-60 group-hover:translate-x-1 transition-transform" />
               </button>
             </div>
          </div>
        )}
  ...
  return (
    <div className="h-screen w-full overflow-hidden flex flex-col font-sans relative bg-[#F8FAFC]">
      ...
      <main className="flex-1 relative overflow-hidden z-10">
        ...
        {currentView === 'RANKS_INFO' && renderHome() /* Redirect Ranks to Home/Tree */}
