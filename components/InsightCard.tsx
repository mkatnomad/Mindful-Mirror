...
export const InsightCard: React.FC<InsightCardProps> = ({ data }) => {
  ...
  const handleShare = async () => {
    const textToShare = `Mindful Mirror: Сложное решение\n\nТема: ${data.topic}\n\nПлюсы:\n${data.pros.map(p => `+ ${p}`).join('\n')}\n\nМинусы:\n${data.cons.map(c => `- ${c}`).join('\n')}`;
    ...
    <div className="w-full my-4 animate-fade-in-up">
      ...
      >
        ...
        <div className="h-28 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 relative overflow-hidden">
             ...
             <div className="absolute bottom-0 left-0 w-24 h-24 bg-indigo-200/20 rounded-full blur-xl translate-y-1/3 -translate-x-1/4"></div>
             
             <div className="absolute inset-0 flex items-center px-6">
                <div>
                  <p className="text-[10px] text-indigo-400 uppercase tracking-widest font-bold">Ваш вопрос</p>
                  <h3 className="text-xl font-bold text-slate-800 mt-1 leading-tight line-clamp-2">{data.topic}</h3>
                </div>
             </div>
        </div>
        ...
        <div className="p-6 relative z-10">
          <div className="space-y-4">
            <div className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100/50">
              <h4 className="text-emerald-600 text-xs font-bold uppercase tracking-wider mb-2">Плюсы</h4>
              <ul className="text-sm text-slate-600 space-y-2">
                 {data.pros.length > 0 ? (
                   data.pros.map((p, i) => <li key={i} className="flex items-start"><span className="mr-2 text-emerald-400 text-lg leading-none">•</span>{p}</li>)
                 ) : (
            ...
            <div className="bg-rose-50/50 p-4 rounded-2xl border border-rose-100/50">
              <h4 className="text-rose-500 text-xs font-bold uppercase tracking-wider mb-2">Минусы</h4>
              <ul className="text-sm text-slate-600 space-y-2">
                 {data.cons.length > 0 ? (
                   data.cons.map((c, i) => <li key={i} className="flex items-start"><span className="mr-2 text-rose-300 text-lg leading-none">•</span>{c}</li>)
                 ) : (
                   <li className="text-slate-400 italic">Нет данных</li>
                 )}
              </ul>
